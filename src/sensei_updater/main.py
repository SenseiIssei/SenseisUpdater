...
def main():
    ...
    if args.schedule:
        ...
        out_path = out_dir.joinpath("last-run.json")
        extra = ["--apps", "--yes", "--report", "json", "--out", str(out_path)]
        ok = sched.create(args.schedule, args.time, args.task_name, extra)
        if ok:
            console.ok(f"Scheduled {args.schedule} task at {args.time}: {args.task_name}")
            console.info("To remove it later, run with --unschedule --task-name \"<name>\"")
        else:
    ...
    if args.quick or args.drivers or args.apps or args.cleanup or args.health or args.startup or args.profile or args.diagnostics:
        ...
        Menu(console, app, drivers, system, cfg)._print_summary(report)
